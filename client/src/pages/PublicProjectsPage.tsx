            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (Array.isArray(data)) {
                        ...
                        setProjects(mapped.filter(p => p.id === 'public-group') as unknown as Project[]);
                    }
                }
            } catch (err) {
                console.error(err);
    ...
    return (
        <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950">
            ...
            <div className="flex-1 ml-[var(--sidebar-width)] transition-[margin] duration-200">
                <Header title="Public Projects" />
                <main className="p-8">
                    <button onClick={() => navigate(-1)} className="mb-6 text-sm font-semibold text-text-gray hover:text-primary">&larr; Back</button>
                    <h1 className="text-3xl font-bold text-text-dark dark:text-white mb-8">Public Projects</h1>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center p-10 bg-white dark:bg-gray-800 rounded-2xl">No public projects found.</div>
                    ) : (
                            ...
                            {projects.map(p => (
                                <div key={p.id} onClick={() => navigate(`/taskflow?projectId=${p.id}`)} className="bg-white dark:bg-gray-800 rounded-2xl p-6 cursor-pointer hover:shadow-lg transition-all border border-gray-200 dark:border-gray-700">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase bg-primary text-white">PUBLIC</span>
                                    </div>
