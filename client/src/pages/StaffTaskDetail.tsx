...
interface AnalysisResult {
    ...
    failed_standards: { rule: string; reason: string; location?: string }[];
    suggestions: string[];
    rejection_details?: {
        issue: string;
        reason: string;
                                    ...
                                    <div className={`rounded-2xl border-2 p-5 flex items-center gap-4 ${
                                        ...
                                        <div>
                                            <p className={`text-xl font-black tracking-wide ${
                                                ...
                                                {result.verdict ?? (result.compliance_score >= 70 ? 'ACCEPTED' : 'NOT ACCEPTED')}
                                            </p>
                                            <p className="text-sm text-text-gray dark:text-gray-400 mt-0.5">
                                                {result.verdict === 'ACCEPTED'
                                                    ? 'This document meets all required quality standards.'
                                                    : 'This document does not meet the required quality standards. See details below.'}
                                            </p>
                                        </div>
                                        <div className="ml-auto text-right">
                                            <p className={`text-3xl font-black ${
                                                result.compliance_score >= 75 ? 'text-green-600 dark:text-green-400'
                                                : result.compliance_score >= 50 ? 'text-yellow-600 dark:text-yellow-400'
                                                : 'text-red-600 dark:text-red-400'
                                            }`}>{result.compliance_score.toFixed(0)}%</p>
                                            <p className="text-xs text-text-gray dark:text-gray-400">Compliance</p>
                                        </div>
                                    </div>

                                    {/* ========== REJECTION DETAILS BOX (red) ========== */}
                                    {result.verdict !== 'ACCEPTED' && result.rejection_details && result.rejection_details.length > 0 && (
                                        <div className="bg-red-50 dark:bg-red-950/40 border-2 border-red-500 dark:border-red-600 rounded-2xl p-5 space-y-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <h3 className="text-sm font-bold text-red-700 dark:text-red-400 uppercase tracking-wide">
                                                    Rejection Reasons — {result.rejection_details.length} Issue{result.rejection_details.length > 1 ? 's' : ''} Found
                                                </h3>
                                            </div>
                                            <div className="space-y-3">
                                                {result.rejection_details.map((detail, i) => (
                                                    <div key={i} className="bg-white dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-700 p-4 space-y-2">
                                                        <div className="flex items-start justify-between gap-2">
                                                            <p className="text-sm font-bold text-red-700 dark:text-red-300">#{i + 1} — {detail.issue}</p>
                                                            <span className="flex-shrink-0 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/40 border border-red-300 dark:border-red-600 px-2 py-0.5 rounded-full whitespace-nowrap">
                                                                📍 {detail.location}
                                                            </span>
                                                        </div>
                                                        <p className="text-xs text-red-600 dark:text-red-400">
                                                            <span className="font-semibold">Why: </span>{detail.reason}
                                                        </p>
                                                        <div className="flex items-start gap-1.5 bg-red-50 dark:bg-red-950/40 rounded-lg p-2">
                                                            <svg className="w-3.5 h-3.5 text-red-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                                                <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
                                                            </svg>
                                                            <p className="text-xs text-red-700 dark:text-red-300">
                                                                <span className="font-semibold">How to fix: </span>{detail.how_to_fix}
                                                            </p>
                                                        </div>
                                                    </div>
                                                ))}
