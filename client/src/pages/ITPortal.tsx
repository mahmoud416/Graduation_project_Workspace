...
type Tab = 'users' | 'tasks' | 'messages' | 'add-user';

interface LoginEntry { timestamp: string; ip?: string }
...
export default function ITPortal() {
    ...
    const [drawerHistory, setDrawerHistory] = useState<LoginEntry[]>([]);
    ...
    useEffect(() => {
        ...
        if (role !== 'it') navigate('/dashboard', { replace: true });
    }, [navigate]);
    ...
    const fetchComments = useCallback(async () => {
        setLoading(true); setError(null);
        try {
            const res = await fetch(`${API_BASE}/it/comments?limit=200`, { headers: authHeaders() });
            if (!res.ok) throw new Error(await res.text());
    ...
    useEffect(() => {
        ...
        if (tab === 'messages') fetchComments();
    }, [tab, fetchUsers, fetchTasks, fetchComments]);

    // Open history drawer — fetch full user detail
    const openHistory = async (user: UserRow) => {
        setDrawerUser(user);
        setDrawerLoading(true);
        try {
            const res = await fetch(`${API_BASE}/it/users/${user._id}`, { headers: authHeaders() });
            if (!res.ok) throw new Error();
            const data: UserRow = await res.json();
            setDrawerHistory(data.login_history ?? []);
        ...
        <div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-6">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                            <table className="w-full text-sm">
                                <thead className="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 uppercase text-xs">
                                    <tr>
                                        {['Name', 'Email', 'Role', 'Status', 'Last Login', 'History', 'Actions'].map(h => (
                    ...
                    {tab === 'messages' && !loading && (
                        <div className="space-y-3">
                            {comments.map(c => (
                                <div key={c._id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm px-5 py-4">
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="font-medium text-gray-800 dark:text-gray-100 text-sm">{c.author_name}</span>
                                        <span className="text-xs text-gray-400">{fmt(c.created_at)}</span>
                                    </div>
                                    <p className="text-xs text-gray-400 mb-2">{c.author_email}</p>
                    ...
                    {tab === 'add-user' && (
                        <div className="max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-sm px-8 py-6">
                            ...
                            <form onSubmit={handleAddUser} className="space-y-4">
                                ...
                                {formSuccess && (
                                    <p className="text-sm text-emerald-600 dark:text-emerald-400">{formSuccess}</p>
                                )}
            ...
            {drawerUser && (
                <div className="fixed inset-0 z-50 flex justify-end">
                    <div className="absolute inset-0 bg-black/40" onClick={() => setDrawerUser(null)} />
                    <div className="relative w-96 bg-white dark:bg-gray-800 h-full shadow-2xl flex flex-col">
                        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                            <div>
                                <h3 className="font-semibold text-gray-800 dark:text-gray-100">{drawerUser.name}</h3>
                        ...
                        <div className="px-5 py-4 overflow-y-auto flex-1">
                            <p className="text-xs font-semibold uppercase text-gray-400 mb-3 tracking-wide">Login History</p>
                            {drawerLoading && (
                                <div className="flex justify-center py-8">
                                    <div className="w-6 h-6 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                                </div>
                            )}
                            {!drawerLoading && drawerHistory.length === 0 && (
                                <p className="text-sm text-gray-400">No login records yet.</p>
                            )}
                            {!drawerLoading && drawerHistory.map((entry, i) => (
                                <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                                    <div className="w-2 h-2 rounded-full bg-indigo-400 shrink-0" />
                                    <span className="text-sm text-gray-700 dark:text-gray-300">{fmt(entry.timestamp)}</span>
                                    {entry.ip && <span className="text-xs text-gray-400 ml-auto">{entry.ip}</span>}
                                </div>
                            ))}
                        </div>
                    </div>
